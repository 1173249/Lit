...
def clean(gamename, imgpath, directory):
    ...
    if multiplayer == '1':
        ...
        os.system(cmd)
        cmd = 'mkdir -p ' + directory + 'trace/function'
        os.system(cmd)
        cmd = 'mkdir -p ' + directory + 'trace/playicon'
        os.system(cmd)
        cmd = 'mkdir -p ' + directory + 'trace/playicon_function'
        os.system(cmd)
...
if __name__ == '__main__':
    ...
    while (True):
        ...
        if (point is not None):
            #adbkit.click(point)
            print("find a function button, please continue")
            adbkit.getevent(diretory+'/trace/function/'+tracefile+'.'+str(ticks))
            continue
        ...
        if multiplayer == '1':
            ...
            adbkit.getevent(diretory+str(player)+'/trace/'+tracefile+'.'+str(ticks))
            ...
            if not cf.has_option(gamename, "playIcons"):
                ...
                if (startpoint is None):
                    ...
                    print("please play next step")
                    adbkit.getevent(diretory+'/trace/playicon/'+tracefile+'.'+str(ticks))
